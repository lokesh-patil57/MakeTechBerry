...
export default function App() {
  ...
  const themeClasses = {
    ...
    footer: isDarkMode ? 'bg-[#111827] border-gray-800' : 'bg-[#FCF8F1] border-gray-200',
    headline: isDarkMode ? 'text-white' : 'text-gray-900',
    linkHeader: isDarkMode ? 'text-gray-400' : 'text-gray-500',
    link: isDarkMode ? 'text-gray-300 hover:text-indigo-400' : 'text-gray-600 hover:text-indigo-600',
  ...
  return (
    <div className={`min-h-[400px] flex flex-col justify-end transition-colors duration-500 ${themeClasses.wrapper}`}>
      
      {/* Spacer / Content Area */}
      <div className="flex-grow flex items-center justify-center p-10 opacity-20">
        <p className={`${themeClasses.bottomText} text-sm font-medium tracking-widest uppercase`}>
          Main Content Section
        </p>
      </div>

      <footer className={`py-12 px-4 sm:px-8 border-t transition-colors duration-500 w-full ${themeClasses.footer}`}>
        <div className="w-full px-6 sm:px-12 lg:px-28">
          ...
          <div className="flex flex-col lg:flex-row items-start justify-between gap-12 mb-12">
            ...
            <div className="max-w-lg w-full flex-shrink-0">
              ...
              <div className="flex gap-3 flex-wrap">
                ...
                <SocialIcon href="/" bgColor="bg-pink-500" ariaLabel="Instagram">
                  <Instagram size={18} />
                </SocialIcon>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-10 gap-x-12 sm:gap-x-20 lg:gap-x-32">
              ...
              <div className="min-w-[120px]">
                <h3 className={`uppercase tracking-widest text-1xl font-bold mb-5 ${themeClasses.linkHeader}`}>
                  Quick Links
                </h3>
          ...
          <div className={`border-t my-8 transition-colors duration-500 ${themeClasses.divider}`}></div>
...
/>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left ">
              <p className={`text-1xl font-medium transition-colors duration-500 ${themeClasses.bottomText}`}>
                © {new Date().getFullYear()}, MakeTechBerry LLP. All rights reserved.
              </p>
            </div>
          </div>
        </div>
